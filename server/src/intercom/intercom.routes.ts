...
export const intercomRoutes = (app: Application) => {
            ...
            await intercom.subscribeToConversation(conversationID, ws, (event) =>
                match(event.type)
                    .with(SocketEvent.USER_MESSAGE, () => {
                        ...
                        intercom?.sendUserReply(userID, conversationID, message, attachmentUrls);
                    })
    ...
    app.post(`/${LiveAgentPlatform.INTERCOM}/conversation`, async (req, res) => {
        ...
        try {
            ...
            console.log(req.body);
            const { userID, conversationID } = await intercom.createConversation(req.body.userID, req.body.name, req.body.email);
    ...
    app.post(`/${LiveAgentPlatform.INTERCOM}/create-ticket`, async (req, res) => {
        ...
        try {
            ...
            const resp = await fetch(`https://api.intercom.io/conversations/${id}/convert`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Intercom-Version": "2.10",
                    Authorization: "Bearer dG9rOjk5NzU4NzJjXzNjZmNfNDhmYl85NzRmXzlhZGFjY2JhNjM2MzoxOjA=",
                },
